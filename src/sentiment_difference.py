"""
sentiment.csv 的情感差分分析（8×8矩阵）/ sentiment.csv の感情差分分析（8×8マトリクス）
- 按用户输出CSV / ユーザーごとにCSVを出力
- 分离 interrupt / current の返答を分離 / interrupt / current の返答を分離
- 保存每次对话的差分 / 各対話の差分を保存
- 按用户计算并保存 interrupt/current 的统计量（max, min, median, mean, Q1, Q3）/ ユーザーごとにinterrupt/currentの統計量（max, min, median, mean, Q1, Q3）を計算・保存
...
# replyType 的映射 / replyType のマッピング
reply_type_map = {
...
for user_id, user_df in df.groupby("userId"):
    ...
    # Plotly可视化 / Plotly可視化
    if stats_results:
        # 按replyType可视化（current和interrupt分别输出HTML）/ replyTypeごとに可視化（currentとinterruptを別々にHTML出力）
        for reply_type_short in stats_df["replyType"].unique():
            ...
            # 保存为HTML文件（current/interrupt分别存放）/ HTMLファイルとして保存（current/interrupt別フォルダ）
