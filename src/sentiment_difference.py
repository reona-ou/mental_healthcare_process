...
- 用Plotly可视化各统计量的8×8矩阵（带颜色热力图，HTML+SVG+PNG输出）/ 各統計量の8×8マトリクスをPlotlyで可視化（色付きヒートマップ、HTML+SVG+PNG出力）
"""
import pandas as pd
import numpy as np
...
for user_id, user_df in df.groupby("userId"):
    ...
    if stats_results:
        ...
        # 保存为HTML+SVG+PNG / HTML+SVG+PNGとして保存
        vis_dir = output_dir / "visualizations"
        vis_dir.mkdir(parents=True, exist_ok=True)
        base = vis_dir / f"{user_id}_statistics"
        export_fig(fig, base)
        print(f"可视化已保存 / 可視化保存: {base}.html")
