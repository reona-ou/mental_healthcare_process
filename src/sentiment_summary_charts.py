...
def create_stats_bar_chart(stats_df, title, output_path):
    ...
    fig = make_subplots(rows=2, cols=3, subplot_titles=['Mean', 'Median', 'SD', 'Max', 'Min'], horizontal_spacing=0.08, vertical_spacing=0.15)
...
def analyze_group(scores_df, prefix, output_dir):
    ...
    create_radar_chart(stats_df, f'{prefix.upper()} — 感情雷达图', output_dir / f'{prefix}_radar.html')
...
if __name__ == "__main__":
    print("Sentiment Summary Charts\n")
    df = pd.read_csv(SENTIMENT_DIR / 'sentiment.csv')
    print(f"Data: {len(df)}, Users: {df['userId'].nunique()}\n")

    print("=== userInput ===")
    analyze_group(df, 'input', SENTIMENT_DIR / 'input_analysis')

    print("\n=== reply ===")
    analyze_group(df, 'reply', SENTIMENT_DIR / 'reply_analysis')
    ...
    create_dominant_emotion_chart(overall_dominant_counts, 'OVERALL — 主導感情分布', overall_dir / 'overall_dominant_emotion.html')
    create_radar_chart(overall_stats, 'OVERALL — 感情雷达图', overall_dir / 'overall_radar.html')
    create_stats_bar_chart(overall_stats, 'OVERALL — 統計指標比較', overall_dir / 'overall_stats_comparison.html')
    create_emotion_boxplot(overall_df, 'OVERALL — 感情スコア分布', overall_dir / 'overall_boxplot.html')

    print(f"\nDone. Output: {SENTIMENT_DIR}")
