...
→ ruri-v3-310m 埋め込み → UMAP削減 → KMeansクラスタリング
→ 統計距離フィルタ → トピック割当
```
...
| seed_topic_list | 8カテゴリ | `BERTOPIC_SEED_TOPICS` | クラスタリング誘導用シード |
| representation_model | MMR (diversity=0.5) | - | キーワード多様性向上 |

### 後処理
1. 短文テキストフィルタ: 分詞後 < 2語 → -1（26件）
2. 統計距離フィルタ: mean + 1.5 * std → -1（9件）
...
| トピック情報 | 補助 | 89 | Topic 5→0, Topic 2/6→1等 |
